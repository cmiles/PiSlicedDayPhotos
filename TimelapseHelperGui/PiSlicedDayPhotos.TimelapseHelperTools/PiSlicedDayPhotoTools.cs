...
public static class PiSlicedDayPhotoTools
{
    public static List<PiSlicedDayPhotoInformation> ProcessDirectory(string directoryPath, IProgress<string> progress)
    {
        ...
        {
            if(++counter == 1 || counter % 50 == 0) progress.Report($"Processing Image {counter} of {allImages.Count}");
    ...
    public static async Task<(string resultFile, bool errors, List<string> runLog)>
        CreateSingleTimeDescriptionTimelapse(List<PiSlicedDayPhotoInformation> photos, int framerate,
            Dictionary<string, int> cameraOrderLookup, string ffmpegExe, IProgress<string> progress,
            bool writeDateTimeString, string dateCaptionDateTimeFormat = "yyyy MMMM", int fontSize = 24)
    {
        ...
        var selectedCameraNames = photos.Select(x => x.Camera).Distinct().ToList();

        progress.Report($"Time Description: {string.Join(", ", selectedTimeDescriptions)}");
        progress.Report($"Cameras: {string.Join(", ", selectedCameraNames)}");
        ...
        if (selectedCameraNames.Count == 1)
        {
        ...
        else
        {
            ...
            foreach (var loopGroup in orderedGroups)
            {
                ...
                foreach (var loopCamera in cameraOrderLookup.OrderBy(x => x.Value))
                {
                    var possibleFile = loopGroup.Photos.FirstOrDefault(x => x.Camera == loopCamera.Key);
                ...
                if (writeDateTimeString)
                    AddTextToImage(destinationFile, loopGroup.ReferenceDateTime.ToString(dateCaptionDateTimeFormat),
                        fontSize, commonDimension.Width * cameraOrderLookup.Count - captionMaxWidth - 10,
        ...
        return (Path.Combine(tempStorageDirectory.FullName, resultFile), runResult.fatalErrors, runResult.runLog);
    }
